...
function generateGrid(width, height, row, col) {
    ...
    if (game.deviceType=="mobile" || game.deviceType=="largeMobile"){
        ...
        // Rendering the mobile-udlr buttons
        for(let j=0; j<4; j++){
...
function drawOnGrid(nrows, ncols, layout=undefined){
    ...
    const renderImage = (coords, m, type, verbose=false) => {
        if (game.isTransfer()){
            if (game.parameters.img_tool == "s"){
                // board.obj[i][j].object  = drawImage(board.paper.object, 'img/shapes/s' + s_order[m-1] + parameters.img_teleporter_t, board.characterimg[i][j].rect);
                board.obj[coords[0]][coords[1]].object = drawImage(board.paper.object, PRE + "s" + game.parameters.s_order[m-1] + game.parameters[type + "_t"] + ".png", board.characterimg[coords[0]][coords[1]].rect, verbose);
            } else {
                // if c
                // board.obj[i][j].object  = drawImage(board.paper.object, 'img/shapes/' + parameters.img_teleporter_t + c_order[m-1] + '.png', board.characterimg[i][j].rect);
                board.obj[coords[0]][coords[1]].object  = drawImage(board.paper.object, PRE + game.parameters[type + "_t"] + game.parameters.c_order[m-1] + ".png", board.characterimg[coords[0]][coords[1]].rect, verbose);
            }
        } else {
            if (game.parameters.img_tool == "s"){
                // board.obj[i][j].object  = drawImage(board.paper.object, 'img/shapes/' + parameters.img_teleporter + c_order[m-1] + '.png', board.characterimg[i][j].rect);
                board.obj[coords[0]][coords[1]].object = drawImage(board.paper.object, PRE + game.parameters[type] + game.parameters.c_order[m-1] + ".png", board.characterimg[coords[0]][coords[1]].rect, verbose);
            } else {
                // if c
                // board.obj[i][j].object  = drawImage(board.paper.object, 'img/shapes/s' + s_order[m-1] + parameters.img_teleporter, board.characterimg[i][j].rect);
                board.obj[coords[0]][coords[1]].object = drawImage(board.paper.object, PRE + "s" + game.parameters.s_order[m-1] + game.parameters[type] + ".png", board.characterimg[coords[0]][coords[1]].rect), verbose;
            }
        }
    }
    ...
    for(let i=0; i<nrows; i++){
        for(let j=0; j<ncols; j++){
            if ((rooms[i][j])==='-'){
                ...
                for (let m=1; m<ms; m++){
                    if (rooms[i][j]===('t' + m)){
                        renderImage([i, j], m, "img_teleporter")
                ...
                for (let m=1; m<ms; m++){
                    ...
                    for (let m=1; m<ms; m++){
                        if (rooms[i][j]===('c' + m)){
                            renderImage([i, j], m, "img_catapult")
                            board.obj[i][j].object.attr({"opacity":1});
                            board.grid[i][j].isFull = true;
                            board.grid[i][j].type   = rooms[i][j];
                        }
                    ...
                    for (let m=1; m<ms; m++){
                        if (rooms[i][j]===('l' + m)){
                            // lever (but I called it lock before I knew this)
                            board.grid[i][j].isLock = true;
                            board.grid[i][j].isWall = true; 
                            renderImage([i, j], m, "img_lever")
                            board.obj[i][j].object.attr({"opacity":1});
                            board.grid[i][j].isFull = true;
                            board.grid[i][j].type   = rooms[i][j];
                        }
...
function drawImage(paper, src, rect, verbose=false) {
...
/**
 * Determines if the next step would be into an outer boundary
 * @param {Object} coords position object with keys `x` and `y`: i.e. {x: 5, y: 6}
 * @returns {boolean} if the room at the provided coords is a wall (and not a door)
 */
function isOutOfBounds(coords){
    if ((coords.x < 0) || (coords.x >= game.parameters.numcols)){
        return true
    } else if ((coords.y < 0) || (coords.y >= game.parameters.numrows)){
        return true
    } else {
        return false
    }
...
function showIntersitial(isRestart=false, isSkip=false){
    ...
    if (player.round.completed == true){
