class Player {
    ...
    saveRound(reachedGoal=false){
        ...
        console.log(`Is restart? ${this.round.resp.tool.includes("restart")}`)
        // Only update rise tracking on pass or skip
        if (this.data.length == 0){
            this.updateRiseTracking(this.round.roundIndex, this.round.endTime);
        }
        // Either this is not a restart, or we've restarted on the final attempt (and thus are skipping)
        else if (!this.round.resp.tool.includes("restart") ||
                (this.round.resp.tool.includes("restart") && this.round.attemptNum == 2)){
            this.updateRiseTracking(this.round.roundIndex, this.round.endTime);
        }

        // Also cache a copy of the last round to assist in reloading
    ...
    isRestart(){
        return (this.data.length != 0 && _.last(this.data).resp.direction.includes("restart"))
    }
    ...
    updateRiseTracking(roundIndex, endTime){
        this.riseTracking.roundNum = this.round.roundIndex + 1
        // this.riseTracking.roundsComplete.push(roundIndex+1)
        if (this.round.completed){
            this.riseTracking.roundComplete.push(endTime)    
        } else {
            this.riseTracking.roundComplete.push(false)
        }
